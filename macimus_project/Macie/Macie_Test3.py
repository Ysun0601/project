...
def analyze_and_reclassify_macie_results(job_id):
    try:
        ...
        for result in sensitive_results:
            ...
            if sensitive_data:
                category = sensitive_data[0].get('category', 'UNKNOWN')
            else:
                category = "UNKNOWN"

            # 닉네임(NICKNAME)을 강제로 LOW로 재분류
            if category == "NICKNAME":
                print(f"Reclassifying NICKNAME finding to LOW severity: {result.get('id', 'Unknown')}")
                classified_results["Low"].append(result)
            else:
                # 기본 심각도 유지
                if severity in classified_results:
                    classified_results[severity].append(result)
                else:
                    print(f"Unknown severity level: {severity}, treating as LOW")
                    classified_results["Low"].append(result)
    ...
    if custom_data_identifier_ids:
        ...
        if job_id:
            ...
            if results:
                ...
                print(json.dumps(results["High"], indent=2, ensure_ascii=False, default=json_serial))
                print("Medium severity results:")
                print(json.dumps(results["Medium"], indent=2, ensure_ascii=False, default=json_serial))
                print("Low severity results:")
                print(json.dumps(results["Low"], indent=2, ensure_ascii=False, default=json_serial))
            else:
