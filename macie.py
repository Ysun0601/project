...
def create_classification_job_with_custom_ids(bucket_name, job_name, custom_ids):
    ...
    try:
        response = macie2.create_classification_job(
            ...
            managedDataIdentifierSelector="ALL",  # 기본 데이터 식별자도 포함
        )
...
def categorize_findings(findings):
    ...
    for finding in findings:
        ...
        for data in sensitive_data:
            for detection in data.get('detections', []):
                if detection['type'] == 'CCN' and detection['count'] > 0:
                    severity = 'HIGH'
                elif detection['type'] == 'SSN' and detection['count'] > 0:
                    severity = 'HIGH'
                else:
                    severity = 'MEDIUM'
